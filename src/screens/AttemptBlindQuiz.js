...
const AttemptBlindQuiz = ({ match }) => {
	...
	const spaceFunction = React.useCallback(
		(event) => {
			if (event.keyCode === 32) {
				...
				else if (quizTitle === 'ERR:QUIZ_ACCESS_DENIED') speak('Access is not granted by the creator.');
				else if (quizTitle === 'ERR:QUIZ_ALREADY_ATTEMPTED') speak('You have already attempted this quiz.');
				else {
	...
	useEffect(() => {
		if (!SpeechRecognition.browserSupportsSpeechRecognition()) alert('Oops, your browser is not supported!');
		else {
	...
	useEffect(() => {
		const fetchQuiz = async () => {
			const res = await fetch('https://quiz-backendapi.herokuapp.com/API/quizzes/join', {
				method: 'POST',
			...
			if (quizData.error) {
				...
				// For Quiz not accessible
				else if (quizData.error === 'ERR:QUIZ_ACCESS_DENIED') speak('Access is not granted by the creator.');
				else if (quizData.error === 'ERR:QUIZ_ALREADY_ATTEMPTED') speak('You have already attempted this quiz.');
			} else {
	...
	const submitQuiz = React.useCallback(async () => {
		...
		try {
			const res = await fetch('https://quiz-backendapi.herokuapp.com/API/quizzes/submit', {
				method: 'POST',
				body: JSON.stringify({
					uid,
					quizId: quizCode,
			...
			setResult(body);
			setShowModal(true);
			if (body.error === 'ERR:QUIZ_ALREADY_ATTEMPTED') speak('The Quiz has already been submitted.');
			else speak(`Your score is ${body.score} out of ${attemptedQuestions.length}`);
			SpeechRecognition.abortListening();
	...
	useEffect(() => {
		const selectOption = (option) => {
			...
			if (!options.includes(option)) {
				if (attemptedQuestions[currentIndex].optionType === 'radio') options[0] = option;
											...
											<input
												...
												checked={
													attemptedQuestions.length
														? attemptedQuestions[index].selectedOptions.includes(option.text)
														: false
												}
